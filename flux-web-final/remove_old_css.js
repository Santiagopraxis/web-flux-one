...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('/* Mobile Menu Toggle - always visible on mobile */')) {
    startIdx = i;
  }
  if (lines[i].includes('/* Badges */') && i > startIdx && startIdx !== -1) {
    endIdx = i;
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  console.log('Removed old mobile CSS by line index: ' + startIdx + ' to ' + endIdx);
} else {
