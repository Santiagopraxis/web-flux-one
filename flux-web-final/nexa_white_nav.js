...
function processDirectory(dir) {
  ...
  files.forEach(file => {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      const startMarker = '.navbar.scrolled .logo-negro, .navbar:has(.mega-dropdown:hover) .logo-negro { display:none; }';
      const endMarker = '.navbar:not(.scrolled) .dropdown-menu:not(.mega-menu-panel) a:hover { color:#0F172A !important; background:#F0F4F8 !important; }';
      
      const startIndex = content.indexOf(startMarker);
      const endIndex = content.indexOf(endMarker);
      
      if (startIndex !== -1 && endIndex !== -1 && startIndex < endIndex) {
        const properCSS = `    .navbar.scrolled { background:rgba(255,255,255,0.95) !important; border-bottom:1px solid rgba(0,0,0,0.05) !important; }
    ...
    .navbar .dropdown-menu:not(.mega-menu-panel) a:hover { color:#0F172A !important; background:#F0F4F8 !important; }`;
        ...
        console.log('Fixed Nexa Navbar CSS in ' + fullPath);
      } else {
